...
# Resolve paths from this script so runs work from any cwd (e.g. repo root).
_APP_DIR = os.path.dirname(os.path.abspath(__file__))

# Configuration
PEOPLE_DIR = os.path.join(_APP_DIR, "people")
ASSETS_DIR = os.path.join(_APP_DIR, "assets")
USER_DATA_FILE = os.path.join(_APP_DIR, "user_database.json")
COUNTDOWN_SECONDS = 3
...
def _open_camera():
    """Open default camera; prefer DirectShow on Windows for more reliable capture."""
...
def auto_face_recognition():
    ...
    if not cap.isOpened():
        ...
        print("  Try another index: set env FACE_CAM_INDEX=1")
    ...
    print(f"Timeout after ~{RECOGNITION_TIMEOUT + COUNTDOWN_SECONDS + 5}s — center your face in the oval, arm's length, good light.\n")
...
def main():
    ...
    print(f"  Faces folder: {PEOPLE_DIR}")
    print(f"  User DB:    {USER_DATA_FILE}")
