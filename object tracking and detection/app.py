...
def main():
    ...
    SWIPE_WINDOW    = 15    # compare last N frames only (avoids still-holding dilution)
    SWIPE_THRESHOLD = 80    # pixels of displacement over that window
    SWIPE_COOLDOWN  = 20    # frames to ignore new swipes after one fires
    MIN_CONF        = 0.1# minimum YOLO confidence to accept detection for swipe
    swipe_cooldown_counter = 0
    ...
    while True:
        ...
        if object_detected:
            ...
            if conf >= MIN_CONF:
                ...
                if swipe_cooldown_counter > 0:
                    swipe_cooldown_counter -= 1

                if len(global_track) >= SWIPE_WINDOW and swipe_cooldown_counter == 0:
                    dx = global_track[-1][0] - global_track[-SWIPE_WINDOW][0]
                    dy = global_track[-1][1] - global_track[-SWIPE_WINDOW][1]
                    ...
                    if abs(dx) > abs(dy) and abs(dx) > SWIPE_THRESHOLD:
                        ...
                        global_track.clear()
                        swipe_cooldown_counter = SWIPE_COOLDOWN
                ...
                # Low confidence — don't add to track but keep existing history
                # so a swipe in progress can still complete
                pass
